...
-- TYPE
ALTER TYPE diskquota.relation_cache_detail DROP ATTRIBUTE RELAM;
-- TYPE END
...
/* ALTER */ CREATE OR REPLACE FUNCTION diskquota.show_worker_epoch() RETURNS bigint STRICT AS '$libdir/diskquota-2.1.so', 'show_worker_epoch' LANGUAGE C; /* ALTER */ CREATE OR REPLACE FUNCTION diskquota.wait_for_worker_new_epoch() RETURNS boolean STRICT AS '$libdir/diskquota-2.1.so', 'wait_for_worker_new_epoch' LANGUAGE C;
...
DROP FUNCTION IF EXISTS diskquota.relation_size_local(reltablespace oid, relfilenode oid, relpersistence "char", relstorage "char", relam oid);
